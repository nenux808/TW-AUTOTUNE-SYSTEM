...
export default async function PublicInvoicePage({ params }: PageProps) {
  try {
    ...
    if (!isServiceRoleConfigured()) {
      console.error("Public invoice access blocked: SUPABASE_SERVICE_ROLE_KEY is missing.");
      return (
        <UnavailableInvoice
          title="Invoice temporarily unavailable"
          message="Secure invoice access is being configured. Please contact TW AUTO TUNE for a copy of this invoice."
        />
      );
    }
        ...
        <div className="mx-auto max-w-6xl">
                        ...
                        return (
                          <tr key={item.id} className="border-t border-slate-200">
                            ...
                            <td className="px-4 py-3 font-semibold text-slate-900">
                              {item.description || "-"}
                              {item.included_note && (
                                <p className="mt-1 text-xs font-normal text-slate-500">{item.included_note}</p>
                              )}
                            </td>
                            <td className="px-4 py-3 text-slate-700">{item.quantity}</td>
                            <td className="px-4 py-3 text-slate-700">
                              {included ? "Included" : money(Number(item.unit_price || 0))}
                            </td>
                            <td className="px-4 py-3 font-semibold text-slate-900">
                              {included ? "Included" : money(lineTotal)}
                            </td>
                          </tr>
                        );
                      })
                ...
                <div className="mt-6 grid gap-4">
                  {job?.customer_complaint && (
                    <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-700">
                      ...
                      <p className="mt-1 whitespace-pre-wrap">{job.customer_complaint}</p>
                    </div>
                  )}
                  {job?.work_completed && (
                    <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-700">
                      <p className="font-semibold text-slate-900">Work Completed</p>
                      <p className="mt-1 whitespace-pre-wrap">{job.work_completed}</p>
                    </div>
                  )}
                  {job?.recommendations && (
                    <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-700">
              ...
              <div className="rounded-2xl bg-slate-950 p-6 text-white shadow-sm">
                ...
                <div className="mt-5 space-y-3 text-sm">
                  ...
                  {Number(invoice.discount_amount || 0) > 0 && (
                    <div className="flex justify-between"><span>Discount</span><strong>-{money(Number(invoice.discount_amount || 0))}</strong></div>
                  )}
          ...
          <section className="mt-6 rounded-2xl bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              {latestInspection ? (
                ...
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">No inspection recorded</span>
              )}
            </div>
              ...
              <>
                {latestInspection.customer_visible_notes && (
                  <div className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-700">
                    ...
                    <p className="mt-1 whitespace-pre-wrap">{latestInspection.customer_visible_notes}</p>
                  </div>
                )}
                        ...
                        <tbody>
                          {attentionItems.map((item: any) => (
                            <tr key={item.id} className="border-t border-slate-200">
                              ...
                              <td className="px-4 py-3">{item.measurement_value ? `${item.measurement_value} ${item.measurement_unit || ""}` : "-"}</td>
                              <td className="px-4 py-3">{item.recommendation || item.mechanic_note || "-"}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
          ...
          <section className="mt-6 rounded-2xl bg-white p-6 shadow-sm">
            <p className="text-sm text-slate-500">
              This customer invoice does not display internal owner details such as part cost, supplier cost, profit margin, or internal notes.
            </p>
          </section>
        </div>
      </main>
    ...
    console.error("Public invoice page failed safely", error);
    return (
      <UnavailableInvoice
        title="Invoice temporarily unavailable"
        message="This invoice could not be loaded right now. Please contact TW AUTO TUNE for assistance."
      />
    );
  }
}
