...
function formatInvoiceNumber(value: any) {
  ...
  return text.toUpperCase().startsWith("INV-") ? text : `INV-${text.padStart(5, "0")}`;
}
...
export default async function PublicInvoicePage({ params }: PageProps) {
  ...
  const { data: rawInvoice, error } = await supabase
    ...
    .select(`
      ...
      jobs(job_number, odometer, customer_complaint, work_completed, recommendations, next_service_odometer, next_service_due_date),
      invoice_items(id, item_type, description, quantity, unit_price, sort_order, visibility, billing_mode, included_in_package, included_note)
    `)
  ...
  const job: any = one(invoice.jobs);
  const customer: any = one(invoice.customers);
  const vehicle: any = one(invoice.vehicles);
  ...
  const latestInspectionRes = invoice.job_id
    ? await supabase
  ...
  const latestInspection = latestInspectionRes.data?.[0] || null;
  ...
  const diagnosticRes = invoice.job_id
    ? await supabase
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-6 grid gap-6 md:grid-cols-3">
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <h2 className="mt-2 text-xl font-bold text-slate-900">{formatInvoiceNumber(invoice.invoice_number)}</h2>
          ...
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <h2 className="mt-2 text-xl font-bold uppercase text-slate-900">{vehicle?.registration || "-"}</h2>
            <div className="mt-4 space-y-2 text-sm text-slate-700">
                    ...
                    invoiceItems.map((item: any) => {
                      ...
                      return (
                        <tr key={item.id} className="border-t border-slate-200">
                          ...
                          <td className="px-4 py-3 text-slate-700">{included ? "Included" : money(item.unit_price)}</td>
          ...
          <aside className="grid gap-6">
            <div className="rounded-2xl bg-slate-950 p-6 text-white shadow-sm">
              ...
              <div className="mt-5 space-y-3 text-sm">
                <div className="flex justify-between"><span>Subtotal</span><strong>{money(invoice.subtotal)}</strong></div>
                {Number(invoice.discount_amount || 0) > 0 && <div className="flex justify-between"><span>Discount</span><strong>-{money(invoice.discount_amount)}</strong></div>}
                <div className="flex justify-between"><span>GST</span><strong>{money(invoice.gst_amount)}</strong></div>
                <div className="border-t border-white/10 pt-4 text-lg"><div className="flex justify-between"><span>Total</span><strong>{money(invoice.total_amount)}</strong></div></div>
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm">
              ...
              <h2 className="mt-1 text-xl font-bold text-slate-900">{settings?.business_name || "TW AUTO TUNE"}</h2>
              <div className="mt-4 grid gap-2 text-sm text-slate-700">
                <p>{[settings?.address_line_1, settings?.address_line_2].filter(Boolean).join(", ") || "-"}</p>
                ...
                <div className="mt-3 rounded-xl bg-slate-50 p-4">
                  ...
                  <p>BSB: {settings?.bank_bsb || "-"}</p>
                  <p>Account No: {settings?.bank_account_number || "-"}</p>
                  <p>Reference: {formatInvoiceNumber(invoice.invoice_number)}</p>
                </div>
                <p>{settings?.invoice_footer_note || "Thank you."}</p>
              </div>
            </div>
          </aside>
        </section>
        ...
        <section className="mt-6 rounded-2xl bg-white p-6 shadow-sm">
          <p className="text-sm text-slate-500">This customer invoice does not display owner-only cost, supplier cost, profit margin, or internal notes.</p>
        </section>
      </div>
    </main>
  );
